...
function whoamiTest (state, bus, done) {
  ...
  function createClient (port) {
    ...
    client.once('error', function (err) {
      ...
      if (state.port === 1776) bus.emit('error', `The default DWebX port (${state.port}) in use, using random port.`)
      else bus.emit('error', `Specified port (${state.port}) in use, using random port`)
      bus.emit('error', `This may impact DWebX's connectivity if you have a firewall.`)
